...
async function checkAndAutoFill(forceShow = false) {
  if (hasAutoFilled && !forceShow) return;

  try {
    const unlockedResponse = await chrome.runtime.sendMessage({ type: 'CHECK_UNLOCKED' });
    if (unlockedResponse && unlockedResponse.unlocked) {
      const response = await chrome.runtime.sendMessage({ type: 'GET_MATCHING_ENTRIES' }) as { matches?: any[] };
 ...
 */
function attachPickerListeners(matches: any[]) {
  const inputs = document.querySelectorAll('[data-eigenvault-detected]');
  inputs.forEach(input => {
    input.addEventListener('focus', () => {
      showCredentialPicker(input as HTMLInputElement, matches);
    });
  });
}
 ...
 */
function showCredentialPicker(input: HTMLInputElement, matches: any[]) {
  ...
  const handleOutsideClick = (e: MouseEvent) => {
    if (!picker.contains(e.target as Node) && e.target !== input) {
      removeCredentialPicker();
...
function detectForms() {
  ...
  if (looseInputs.length > 0) {
    ...
    looseInputs.forEach(input => {
      const fieldInfo = determineFieldType(input as HTMLInputElement);
      if (fieldInfo) virtualForm.fields.push(fieldInfo);
    });

    if (virtualForm.fields.length > 0) {
      detectedForms.set(-1, virtualForm as DetectedForm);
    }
  }
}
...
function fillCredentials(username: string, password: string) {
  ...
  if (!filledTypes.has('password') && password) {
    const passInputs = document.querySelectorAll('input[type="password"]') as NodeListOf<HTMLInputElement>;
 ...
 */
function fillLooseInputs(username: string, password: string) {
  const userInputs = document.querySelectorAll('input:not([type="password"]):not([type="hidden"]):not([type="submit"]):not([type="button"])') as NodeListOf<HTMLInputElement>;
  const passInputs = document.querySelectorAll('input[type="password"]') as NodeListOf<HTMLInputElement>;
  
  let filled = 0;
  
  // Try to find the "best" username input
  let bestUserInput: HTMLInputElement | null = null;
  ...
  if (filled > 0) showFillNotification(filled);
}
...
function showFillNotification(fieldCount: number) {
  ...
  if (document.getElementById('eigenvault-notification')) return;
...
function observeDOM() {
  const observer = new MutationObserver((mutations) => {
    ...
    for (const mutation of mutations) {
      if (mutation.addedNodes.length > 0) {
        for (const node of mutation.addedNodes) {
          if (node.nodeName === 'FORM' || (node as Element).querySelector?.('form') || node.nodeName === 'INPUT') {
            shouldDetect = true;
            break;
          }
        }
    ...
    if (shouldDetect) {
      ...
      (window as unknown as { _evDetectTimeout?: number })._evDetectTimeout = setTimeout(() => {
        detectForms();
        disableBrowserAutofill();
        if (!hasAutoFilled) checkAndAutoFill();
      }, 500);
    }
  });
